...
from modules import QLinear, QParameter
from sqpr_engine_experimental import SPQRUtil, Quantizer, quantize
from distillation import cache_teacher_outputs, train_student
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for name in subset:
                ...
                # replace original layer by quantized version
                qparam = QParameter(
                    quantized.qweight, 
                    quantized.scale, 
                    quantized.zero, 
                ...
                qlayer = QLinear(qparam, subset[name].bias)
                # surgery
                if '.' in name:
                    ...
                    setattr(layer, name, qlayer)
                # OUTLIER STATS per module:
                normal_outliers_count = quantized.outliers._nnz()

                stats_payload[f"n_{name}_ol_share"] = round(normal_outliers_count / quantized.qweight.numel(), 6)

                normal_outlier_count += normal_outliers_count
                w_count += quantized.qweight.numel()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Where to extract finetune data from."
    )
    parser.add_argument(
        "--finetune_nsamples", 
        type=int, 
        default=128, 
        help="Number of finetune data samples."
    )
    parser.add_argument(
        "--finetune_batch_size", 
        type=int, 
    ...
    if args.finetune:
        print('\n============ Finetuning... ============')
        # prepare float teacher
        teacher_model = get_llama(args.model_path).train(False)
        # prepare loaders for finetuning
        dataloader, _ = get_loaders(
            args.finetune_dataset, 
            nsamples=args.finetune_nsamples, 
            seed=args.seed, 
            model_path=args.model_path,
            seqlen=model.seqlen
        )
        teacher_outputs = cache_teacher_outputs(
            teacher_model, 
            [item[0] for item in dataloader], 
            args.target_feature,
            'model.layers', 
        ...
        del teacher_model
        torch.cuda.empty_cache()
        # put quantized model on device (for training)
        model = model.to(device)
        for param_name, param in model.named_parameters():
            if 'outliers' not in param_name:
                param.requires_grad = False
        train_student(model, dataloader, teacher_outputs, args)
        # unwrap PEFT model
        model = model.get_base_model()
        print("\n============ Evaluating perplexity after finetuning... ============")
        datasets = ["wikitext2", "ptb", "c4"]
        if args.new_eval:
            datasets = ["wikitext2", "ptb-new", "c4-new"]
        for dataset in datasets:
