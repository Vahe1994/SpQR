...
CUDA_KERNEL = load(
    name="spqr_torch_lib",
    sources=[os.path.join(CUDA_FOLDER, "spqr_torch_lib.cpp"), os.path.join(CUDA_FOLDER, "spqr_cuda_kernel.cu")],
    extra_cuda_cflags=["-O3", "-arch=native"],
)

torch.library.define(
    "spqr_torch_lib::spqr_mul",
    "(int m, int n, int bits, int beta1, int beta2, Tensor buff0, Tensor row_offsets, Tensor col_vals, int nnz, Tensor x, int f, Tensor Y, Tensor(Y!) out) -> ()",
)
