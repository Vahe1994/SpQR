...
class SPQRUtil:
    ...
    def quantize(
        ...
        **kwargs,
        ...
        qweight = torch.zeros_like(weight, dtype=torch.uint8)
        scale = torch.zeros((out_dim, in_dim // groupsize), device=weight.device, dtype=weight.dtype)
        zero = torch.zeros((out_dim, in_dim // groupsize), device=weight.device, dtype=torch.uint8)
        ...
        for block_start in block_start_iter:
            ...
            for column_index in range(block_start, block_end):
                if column_index % groupsize == 0:
                    ...
                    scale[:, in_group_index] = quantizer.scale.flatten()
                    zero[:, in_group_index] = quantizer.zero.flatten()

                    del group_weight

                weight_i_quantized, q_i = quantize(
                    weight[:, column_index].unsqueeze(1), 
                    quantizer.scale, quantizer.zero, quantizer.maxq, return_int=True
                )
                weight_i_quantized = weight_i_quantized.reshape_as(weight[:, column_index])
                q_i = q_i.reshape_as(weight[:, column_index])
                ...
                if unstructured_outlier_threshold != float("inf"):
                    ...
                    weight_i_quantized_wo_outliers, q_i = quantize(
                        (weight[:, column_index] * (1 - is_outlier)).unsqueeze(1), 
                        quantizer.scale, quantizer.zero, quantizer.maxq, return_int=True
                    )
                    weight_i_quantized_wo_outliers = weight_i_quantized_wo_outliers.reshape_as(weight[:, column_index])
                    q_i = q_i.reshape_as(weight[:, column_index])
                    weight_i_quantized = (
        ...
        # gather outliers
        if unstructured_outlier_threshold != float("inf"):
            outlier_ix = torch.nonzero(unstructured_outlier_mask).T
            outlier_vals = weight[unstructured_outlier_mask].to(dtype)
            outliers = torch.sparse_coo_tensor(outlier_ix, outlier_vals, weight.shape)
        else:
            outliers = None

        # prepare quantization result
...
class QuantizationResult(NamedTuple):
    ...
    outliers: Optional[torch.FloatTensor] = None
